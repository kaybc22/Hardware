...
for i in $(mst status -v | awk '/ConnectX|BlueField3/ {print$3}' | grep -v '\.1$')
do echo $i 
sudo lspci -vvs $i | egrep "V2|V3|SN"
done
}
...
function Get_NIC_Info_enhance() {
     ...
     #sudo mlxlink -d $i | egrep "State|Recommendation"
        sudo lspci -vvvs $bus_id | egrep "V2|V3|SN"
     done
}
...
for i in $(ls /dev/ | grep -i rsh) 
do echo $i 
sudo cat /dev/$i/misc | grep DEV
done
}
